	...
	if (preg_match("/^(\d{1,10},)*(\d{1,10})$/",$sqlin))
	{
		...
		while($row = $db->fetch_array($result))
		{
			...
			else
			{
				...
				$return=$return+$db->affected_rows();
				if (!$db->query("update  ".table('jobs')." SET setmeal_deadline='{$deadline}'  WHERE uid='{$row['uid']}'")) return false;
				if (!$db->query("update  ".table('jobs_tmp')." SET setmeal_deadline='{$deadline}'  WHERE uid='{$row['uid']}'")) return false;
			}
		}
	}
	return $return;
